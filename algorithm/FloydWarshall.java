...
public final class FloydWarshall {
    /**
     * Given a directed, weighted graph G, runs the Floyd-Warshall algorithm
     * on that graph and produces a new graph with an edge (i, j) between each
     * pair of nodes whose cost is the cost of the shortest path from i to j
     * in the source graph.
     *
    ...
    public static <T> DirectedGraph<T> shortestPaths(DirectedGraph<T> graph) {
         ...
         *
         * This first loop sets everything to infinity.  Edges are considered
         * in the next loop.
         */
         ...
         * between nodes and the integers they correspond to, this loop is a
         * bit tricky.  The outer loop entries are Node -> Index pairs, while
         * the inner loop entries are Node -> Distance pairs.
         */
        for (Map.Entry<T, Integer> source: nodes.entrySet()) {
            for (Map.Entry<T, Double> edge: graph.edgesFrom(source.getKey()).entrySet()) {
                /* This next line is also hairy.  The first part determines
                 * what indices to use when caching the edge in the table.
                 * The index of the first node is the value associated with
                 * the source node in the outer iteration.  To get the index
                 * of the edge's endpoint, we have to look up the key of the
                 * edge in the node table.  Finally, the length of this edge
                 * is the value in the inner entry.
                 */
                values[source.getValue()][nodes.get(edge.getKey())] = edge.getValue();
            }
        }
        ...
        /* At this point we're ready to begin the iteration.  We'll start off
         * by creating a scratch array to hold the intermediary values at each
         ...
         * modifying.
         *
         * As an optimization, we only allocate this array once.  We'll flip
         * back and forth between this buffer and the values array, much in
         * the same way that double-buffering works in graphical displays.
         */
        ...
        /* We now have the shortest paths; all that's left now is converting
         * it back to a graph.
         */
    ...
    private static <T> Map<T, Integer> enumerateNodes(DirectedGraph<T> graph) {
         ...
         * is added, since this is also a counter telling us the next number
         * we can use.
         */
        for (T node: graph)
     ...
     */
    @SuppressWarnings("unchecked")
    private static <T> DirectedGraph<T> distancesToGraph(double[][] values,
                                                         Map<T, Integer> nodeToIndex) {
        /* In order to invert the table back to a set of values, we're going
         * to need to flip the map around to get back an association of ints
         * to nodes.
         */
